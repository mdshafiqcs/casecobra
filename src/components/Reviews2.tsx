  ...
  >
    {
      reviews.concat(reviews).map((imgSrc, index) => {
        return <Review imgSrc={imgSrc} key={index} className={reviewClassName?.(index % reviews.length)} />
      })
    }
  </div>
}
...
const Reviews2: FC = ({}) => {
  return <MaxWidthWrapper className='relative max-w-5xl'>
    <img src="/what-people-are-buying.png" alt="" className='absolute select-none pointer-events-none hidden xl:block -left-32 top-1/3' />
    <ReviewGrid />
  </MaxWidthWrapper>
}
