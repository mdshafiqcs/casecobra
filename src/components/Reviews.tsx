...
function Review({ imgSrc, className, ...props }:ReviewProps) {
  ...
  const POSSIBLE_AMINATION_DELAYS = [
    '0s',
    '0.1s',
    '0.2s',
    '0.3s',
  ...
  const animationDelay = POSSIBLE_AMINATION_DELAYS[Math.floor(Math.random() * POSSIBLE_AMINATION_DELAYS.length)];
...
function ReviewGrid() {
  ...
  return <div ref={containerRef} className='overflow-hidden relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3' >
    {
      isInView && <>
      <ReviewColumn 
...
const Reviews: FC = ({}) => {
  return <MaxWidthWrapper className='relative max-w-5xl'>
    <img src="/what-people-are-buying.png" alt="" area-hidden="true" className='absolute select-none pointer-events-none hidden xl:block -left-32 top-1/3' />
    <ReviewGrid />
  </MaxWidthWrapper>
}
